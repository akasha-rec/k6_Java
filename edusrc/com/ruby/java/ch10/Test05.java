...
public class Test05 {
	...
	public static void main(String[] args) {
		...
		for (String element : list) { //확장형 for문 돌 때마다 커서
			System.out.print(element + "\t");
		} // 서울   북경   상해

		System.out.println();
		
		list.add(1, "LA"); // [서울, LA, 북경, 상해]
		print(1, list);
		list.addFirst("런던"); // [런던, 서울, LA, 북경, 상해]
		print(2, list);
		list.addLast("서울"); // [런던, 서울, LA, 북경, 상해, 서울]
		print(3, list);

		list.offer("파리"); // add와 비슷 >[런던, 서울, LA, 북경, 상해, 서울, 파리]
		print(4, list);
		list.offerFirst("로마");
		print(5, list);
		list.offerLast("베른"); //[로마, 런던, 서울, LA, 북경, 상해, 서울, 파리, 베른]
		print(6, list);
		...
		//asList > 배열을 리스트로 만들어주고
		List<String> list2 = Arrays.asList("북경", "상해"); //[북경, 상해]
		list.addAll(list2);
		//list.sort();/Linked List의 sort는 지원되지 않음
		
		//toArray > 리스트를 배열로 만들어줌
	...
	static void print(int n, List<String> list) {
		System.out.println(n + " : " + list);//list.toString() 호출 > [ , , ] 형태로 출력
	}
}