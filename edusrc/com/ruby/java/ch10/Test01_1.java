...
public class Test01_1 {
	public static void main(String[] args) {
		Object arr[] = new Object[10]; //상위 객체라서 어느 타입이던 가능
		String s = "java";
		Integer i1 = 123;
		arr[0] = s;
		arr[1] = i1;
//		int len = arr[0].length(); //Object에 length()가 없다 < 함수 body = Class를 봐야 < 수신 객체.메서드(함수 호출)
		int len = ((String) arr[0]).length();
		
		//↓ heap에 연속된 메모리 생성 > Object 객체로 간주 
		ArrayList lst = new ArrayList();//모든 타입의 객체들을 저장할 수 있다 그러나 비추다! - 이유는?
		String s2 = "java";
		Integer i2 = 123; //boxing(by wrapper 클래스)
		...
		//↓ <> : generic 제한? 뒤의 <>은 중복이라서 생략가능
		ArrayList<String> list = new ArrayList<>();//String 객체로 제한 > 바람직한 형태이다 

		//list.toString(); - AbstractCollection 클래스?
		...
		System.out.println("list.size() = " + list.size()); //list.size() - 길이
		...
		// boolean remove(Object o) Array Class가 알아서 당긴다
		...
		// boolean addAll(Collection<? extends E> c) - ?는 wild card 사용 *** 중요 < 나중에 써...
		list.addAll(list2);
		print(13, list);

		// boolean containsAll(Collection<?> c) Class의 모든 컬렉션 가능?
