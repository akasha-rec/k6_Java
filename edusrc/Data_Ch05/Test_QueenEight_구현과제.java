...
import Data_Ch04.objectStack.EmptyGenericStackException;
import Data_Ch04.objectStack.OverflowGenericStackException;
...
public class Test_QueenEight_구현과제 {
	public static void EightQueen(int[][] d) {
		...
		while (true) { //175p
			...
			if (s.isEmpty() != true) {
				n = s.pop(); //pop한 위치를 사용해서 다음 열 조사 > 더 이상 없으면 이전 행으로 돌아가
...
//8 Queen 문제는 각 행에, 각 열에, 각 대각선에 모두 퀸이 하나씩 놓여야 해결 > 목표...
	public static boolean checkRow(int[][] d, int crow) {//배열 d에서 행 crow에 퀸을 배치할 수 있는지 조사
		for (int x = 0; x < d.length; x++) {//각 행을 반복해서 조사하고 싶다.
			if (d[crow][x] == 1) //그래서 어떤 행에 이미 퀸이 있다면 >> 어떤 열에 퀸이 있는지 조사 > 그래서 행은 고정
				return false; //못 놓는다.
		}
		return true; //놓을 수 있다.
	}
	...
	public static boolean checkCol(int[][] d, int ccol) {//배열 d에서 열 ccol에 퀸을 배치할 수 있는지 조사
		for (int y = 0; y < d[0].length; y++) { //각 열을 반복해서 조사하고 싶다
			if (d[y][ccol] == 1) //그래서 어떤 열에 이미 퀸이 있다면 >> 어떤 행에 퀸이 있는지 조사 > 그래서 열은 고정
				return false; //못 놓는다
		}
		return true; //놓을 수 있다.
	}
	...
	public static boolean checkDiagSW(int[][] d, int cx, int cy) { // x++, y-- or x--, y++ where 0<= x,y <= 7
		while (0 <= cx && cx <= 7 && 0 <= cy && cy <= 7) {//경계를 넘으면 안 되니까
